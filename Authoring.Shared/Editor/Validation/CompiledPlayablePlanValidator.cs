...
namespace Supercent.PlayableAI.Generation.Editor.Validation
{
    ...
    public static class CompiledPlayablePlanValidator
    {
        ...
        {
            return catalog != null && (catalog.FeatureDescriptors ?? new FeatureDescriptor[0]).Length > 0;
        }
...
private static void ValidateEnvironmentSourceImageReferences(
    ...
    HashSet<string> declaredSourceImageIds,
        ...
        private static void ValidateFeatureLayouts(
            ...
            CompiledPlayablePlanValidationResult result)
        {
            FeatureJsonPayload[] safeLayouts = featureLayouts ?? new FeatureJsonPayload[0];
            var seen = new HashSet<string>(StringComparer.Ordinal);
            for (int i = 0; i < safeLayouts.Length; i++)
            {
                ...
                if (!string.IsNullOrWhiteSpace(layout.json) && !LooksLikeJsonObject(layout.json))
                    ...
                    ValidateDescriptorDrivenLayoutBakeParity(layout, featureType, targetId, spawnLookup, catalog, "featureLayouts[" + i + "]", result);
        ...
        private static void ValidateDescriptorDrivenLayoutBakeParity(
            ...
            CompiledPlayablePlanValidationResult result)
        {
            if (string.IsNullOrEmpty(featureType) || string.IsNullOrEmpty(targetId))
                return;
        ...
        private static void ValidateRequiredTargetDesignCapabilities(
            ...
            CompiledPlayablePlanValidationResult result)
        {
            string[] requiredCapabilities = descriptor != null && descriptor.layoutRequirements != null
                ? descriptor.layoutRequirements.requiredDesignCapabilities ?? new string[0]
                : new string[0];
            if (requiredCapabilities.Length == 0)
                return;

            if (!TryResolveSpawn(targetId, spawnLookup, out CompiledSpawnData spawn))
                return;

            if (spawn == null || string.IsNullOrWhiteSpace(spawn.objectId))
            {
                Fail(result, label + " targetId '" + targetId + "'의 compiled spawn objectId가 비어 있어 requiredDesignCapabilities를 확인할 수 없습니다.");
                return;
            }

            if (!catalog.TryResolveGameplayPrefab(spawn.objectId.Trim(), spawn.designIndex, out GameObject prefab, out _) || prefab == null)
            {
                Fail(result, label + " targetId '" + targetId + "'의 catalog design prefab을 해석할 수 없어 requiredDesignCapabilities를 확인할 수 없습니다.");
                return;
            }

            if (!PortablePrefabMetadataUtility.TryGetMetadata(prefab, out CatalogPrefabMetadata metadata) || metadata == null)
            {
                Fail(result, label + " targetId '" + targetId + "'의 prefab metadata를 읽지 못해 requiredDesignCapabilities를 확인할 수 없습니다.");
                return;
            }

            for (int i = 0; i < requiredCapabilities.Length; i++)
            {
                string capability = FeatureDescriptorUtility.Normalize(requiredCapabilities[i]);
        ...
        private static bool TryResolveSpawn(
            string targetId,
            Dictionary<string, CompiledSpawnData> spawnLookup,
            out CompiledSpawnData spawn)
        {
            spawn = null;
            if (string.IsNullOrEmpty(targetId) || spawnLookup == null)
                return false;

            return spawnLookup.TryGetValue(targetId, out spawn) && spawn != null;
        }

        private static void ValidateEditorPreviewPathLayout(
            FeatureJsonPayload layout,
            FeatureDescriptor descriptor,
            string targetId,
            string label,
            CompiledPlayablePlanValidationResult result)
        {
            ...
            string cellsField = FeatureDescriptorUtility.Normalize(preview.path != null ? preview.path.cellsField : string.Empty);
            ...
            CompiledPlayablePlanValidationResult result)
        {
            FeatureEditorPreviewDescriptor preview = descriptor != null ? descriptor.editorPreview : null;
            string renderer = FeatureDescriptorUtility.Normalize(preview != null ? preview.renderer : string.Empty);
            if (!string.Equals(renderer, FeatureDescriptorContracts.EDITOR_PREVIEW_RENDERER_BOUNDS, StringComparison.Ordinal))
                return;
            ...
            bool hasAnyDeclaredField = false;
            for (int i = 0; i < bounds.Length; i++)
            {
                ...
                hasAnyDeclaredField = true;
                if (HasPositiveWorldBoundsObject(layout != null ? layout.json : null, field))
                    return;
            }

            if (hasAnyDeclaredField)
                Fail(result, label + " targetId '" + targetId + "'의 descriptor editorPreview.bounds field 중 하나는 hasWorldBounds=true, positive worldWidth/worldDepth를 가져야 합니다.");
        }
        ...
        private static bool TryCountJsonArrayItems(string json, string propertyName, out int count)
        {
            count = 0;
            ...
            for (int i = valueStart + 1; i < end; i++)
            {
                char ch = source[i];
                if (inString)
                {
        ...
        private static bool HasPositiveWorldBoundsObject(string json, string propertyName)
        {
            if (!TryFindJsonPropertyValueStart(json, propertyName, out int valueStart))
                return false;
            ...
            string objectJson = source.Substring(valueStart, end - valueStart + 1);
            return TryReadJsonBoolean(objectJson, "hasWorldBounds", out bool hasWorldBounds) &&
                   hasWorldBounds &&
                   TryReadJsonNumber(objectJson, "worldWidth", out float worldWidth) &&
                   TryReadJsonNumber(objectJson, "worldDepth", out float worldDepth) &&
                   worldWidth > 0f &&
                   worldDepth > 0f;
        }

        private static bool TryFindJsonPropertyValueStart(string json, string propertyName, out int valueStart)
        {
            valueStart = -1;
            string source = json ?? string.Empty;
            string normalizedProperty = FeatureDescriptorUtility.Normalize(propertyName);
            if (string.IsNullOrEmpty(normalizedProperty))
                return false;

            string quotedProperty = "\"" + normalizedProperty + "\"";
            int searchIndex = 0;
            while (searchIndex < source.Length)
            {
                int propertyIndex = source.IndexOf(quotedProperty, searchIndex, StringComparison.Ordinal);
                if (propertyIndex < 0)
                    return false;

                int colonIndex = propertyIndex + quotedProperty.Length;
        ...
        {
            if (source == null || start < 0 || start >= source.Length || source[start] != open)
                return -1;
        ...
        private static bool TryReadJsonBoolean(string json, string propertyName, out bool value)
        {
            ...
            string source = json ?? string.Empty;
            if (MatchesJsonLiteral(source, valueStart, "true"))
            {
                value = true;
                return true;
            }

            if (MatchesJsonLiteral(source, valueStart, "false"))
                return true;

            return false;
        }

        private static bool TryReadJsonNumber(string json, string propertyName, out float value)
        {
            value = 0f;
            if (!TryFindJsonPropertyValueStart(json, propertyName, out int valueStart))
                return false;
            ...
            return float.TryParse(
                source.Substring(valueStart, end - valueStart),
                System.Globalization.NumberStyles.Float,
                System.Globalization.CultureInfo.InvariantCulture,
                out value);
        }

        private static bool MatchesJsonLiteral(string source, int start, string literal)
        {
            if (source == null || start < 0 || literal == null || start + literal.Length > source.Length)
                return false;
