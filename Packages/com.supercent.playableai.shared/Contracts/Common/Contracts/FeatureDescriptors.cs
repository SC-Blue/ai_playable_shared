...
namespace Supercent.PlayableAI.Common.Contracts
{
    ...
    internal static class BuiltinFeatureDescriptorPackLoader
    {
        private const string BuiltinPackRelativePath = "Contracts/Common/FeatureDescriptors/Builtin";

        public static FeatureDescriptor[] LoadAll()
        {
            string directory = FindBuiltinDescriptorDirectory();
            if (string.IsNullOrWhiteSpace(directory) || !Directory.Exists(directory))
                return Array.Empty<FeatureDescriptor>();

            string[] paths = Directory.GetFiles(directory, "*.feature.json", SearchOption.TopDirectoryOnly);
            Array.Sort(paths, StringComparer.OrdinalIgnoreCase);

            var descriptors = new List<FeatureDescriptor>();
            for (int i = 0; i < paths.Length; i++)
                LoadDescriptor(paths[i], descriptors);

            FeatureDescriptor[] result = descriptors.ToArray();
            if (!FeatureDescriptorValidator.TryValidate(result, out string error))
                throw new InvalidOperationException("built-in feature descriptor pack이 유효하지 않습니다: " + error);

            return result;
        }

        private static void LoadDescriptor(string path, List<FeatureDescriptor> descriptors)
        {
            string json = File.ReadAllText(path);
            FeatureDescriptor descriptor = JsonUtility.FromJson<FeatureDescriptor>(json);
            if (!FeatureDescriptorUtility.IsValid(descriptor))
                throw new InvalidOperationException("built-in feature descriptor를 읽지 못했습니다: " + path);

            descriptor.isBuiltin = true;
            descriptors.Add(FeatureDescriptorUtility.Clone(descriptor));
        }

        private static string FindBuiltinDescriptorDirectory()
        {
            string[] roots =
            {
            ...
            {
                string found = FindFromRoot(roots[i]);
        ...
        private static string FindFromRoot(string root)
        {
            string current = string.IsNullOrWhiteSpace(root) ? string.Empty : Path.GetFullPath(root);
            while (!string.IsNullOrWhiteSpace(current))
            {
                string externalCandidate = Path.Combine(current, "External", "ai_playable_shared", "Packages", "com.supercent.playableai.shared", BuiltinPackRelativePath);
                if (Directory.Exists(externalCandidate))
                    return externalCandidate;

                string packageCandidate = Path.Combine(current, "Packages", "com.supercent.playableai.shared", BuiltinPackRelativePath);
    ...
    public static class BuiltinFeatureDescriptorProvider
    {
        public static FeatureDescriptor[] GetBuiltins()
        {
            return FeatureDescriptorUtility.CloneArray(BuiltinFeatureDescriptorPackLoader.LoadAll());
        }

        public static FeatureDescriptor[] GetBuiltins(string[] availableFeatureTypes)
        {
            var available = new HashSet<string>(StringComparer.Ordinal);
            string[] safeFeatureTypes = availableFeatureTypes ?? Array.Empty<string>();
            for (int i = 0; i < safeFeatureTypes.Length; i++)
            {
                string featureType = FeatureDescriptorUtility.Normalize(safeFeatureTypes[i]);
                if (!string.IsNullOrWhiteSpace(featureType))
                    available.Add(featureType);
            }

            FeatureDescriptor[] builtins = GetBuiltins();
            if (available.Count == 0)
                return FeatureDescriptorUtility.CloneArray(builtins);

            var filtered = new List<FeatureDescriptor>();
            for (int i = 0; i < builtins.Length; i++)
            {
                string featureType = FeatureDescriptorUtility.Normalize(builtins[i] != null ? builtins[i].featureType : string.Empty);
                if (available.Contains(featureType))
                    filtered.Add(FeatureDescriptorUtility.Clone(builtins[i]));
            }
